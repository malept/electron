...
exports.getNugetPackageID = function () {
  const nugetDir = path.dirname(updateExe)
  // Strip the higher directory from the nugetDirectory so we are left with just
  // the containing folder name.  Then strip the proceeding slash
  return nugetDir.replace(path.dirname(nugetDir), '').replace(/^\\/g, '')
}

exports.getExeName = function () {
  return path.basename(process.execPath).replace(/\.exe$/g, '')
}
